...
Als zu Beginn des 20. Jahrhunderts erste Haushaltsgeräte wie der Kühlschrank oder der Staubsauger erfunden wurden entstand gleichzeitig der Wunsch, dass die Geräte ihre Aufgaben auch autonom und in Absprache erledigen könnten.\myfootcite[Vgl.][]{smart_home_history_iotevolutionworld}
...
Der \ac{ECHO} IV fungierte damals als Wecker, Fernbedienung für den Fernseher und die Musikanlage, sowie als Uhr.
Sutherland plante auch mit dem \ac{ECHO} IV die hauseigene Beleuchtung zu steuern.\myfootcite[Vgl.][]{smart_home_history_echoiv}

Nachdem 1970 die ersten Mikrocomputer von dem Unternehmen Pico entwickelt wurden, beschäftigte sich Pico unter dem Projektnamen X10 mit der Idee, Beleuchtung und andere Haushaltsgeräte über deren Stromkabel zu steuern.
1978 wurden die Ergebnisse des Projekts erstmals unter dem Produktnamen X10 verkauft.
...
Dieser Trend setzte sich bis heute fort, auch weil zwei Jahre später Amazon Alexa vorgestellt wurde, was das Thema Smart Home normalen Konsumenten näher brachte.\myfootcite[Vgl.][]{alexa_release}
\autoref{fig:googletrendssmarthome} zeigt die Popularität des Smart Home Trends seit 2004, gemessen anhand Google-Suchanfragen.
Je höher der Wert auf der Y-Achse, desto populärer war der Trend, relativ im Vergleich zu allen anderen Suchanfragen.\myfootcite[Vgl.][]{google_trends_docs}
...
Anschließend werden eine Auswahl an Geschäftsmodellen vorgestellt, welche vorhergehende Erläuterungen monetarisieren.
...
Die Interaktion lassen sich in Eingaben und Ausgaben teilen.
...
Bspw. könnte über das Ethernet-Kabel, welches zu einer außen am Haus montierten Überwachungskamera führt, auf das gesamte Heimnetzwerk zugegriffen und mitgelauscht werden.
...
Kommunikation in \ac{MQTT} findet also nach dem Push-Prinzip statt, d.h. ein Client sendet keine Anfragen, sondern erhält bei einem Ereignis eine Benachrichtigung.
...
Da \ac{MQTT} ein offener und frei verfügbarer Standard ist, lässt sich das Protokoll auch vermehrt in \ac{DIY}-Projekten finden.\myfootcite[Vgl.][]{bruhautomation_esp_sensor}
...
\begin{wrapfigure}{r}{0.35\textwidth}
	...
	\includegraphics[width=0.32\textwidth]{zigbee-stack}
...
So wird eine Nachricht etwa von einer Glühbirne, zu einer Steckdose, über einen Kühlschrank hin zu dem sog. ZigBee Hub gesendet, welches die Schnittstelle zur Interaktion mit ZigBee Geräten darstellt und somit die Reichweite und Signalstärke des ZigBee Netzwerks erhöht.
...
Der Beleuchtungssystemanbieter Signify nutzt ZigBee um Philips Hue Beleuchtungssyteme zuverlässig und kabellos zu vernetzen.
...
Abschließend bleibt anzumerken, dass aufgrund der \ac{P2P}-Architektur maximale Datenübertragungsraten von bis zu 250 kbit/s erreicht werden können.\myfootcite[Vgl.][]{zigbee_faq}
...
Beide Systeme ermöglichen es Batterie betriebenen Geräten über mehrere Jahre hinweg Nachrichten ohne Batterie-Wechsel senden und empfangen zu können.\myfootcite[Vgl.][]{smartcave_zwave_vs_zigbee}
...
Noch kann nicht abgeschätzt werden, ob sich Z-Wave, ZigBee oder ein dritter \ac{IoT}-Standard durchsetzen wird.
Nach \autoref{tab:zwave_vs_zigbee} sind Z-Wave und ZigBee was beteiligte Firmen, kompatible und verkaufte Produkte anbelangt etwa gleichauf.
Auf technischer Ebene ähneln sich die Standards.
...
Darüber hinaus werden \ac{IoT}-Geräte dem Internet oft direkt ausgesetzt, was zur Folge hat, dass Dritte sich über eben diese Geräte Zugriff auf ein Smart Home System verschaffen können, da \ac{IoT}-Geräte oft nicht richtig konfiguriert sind, oder Sicherheitslücken aufweisen, welche nicht vom Hersteller geschlossen werden.\myfootcite[Vgl.][]{iot_software_problem}
...
Oft verfügen \ac{IoT}-Geräte über ein \ac{API}, welches direkt angesprochen werden kann.
Das Unternehmen Xiaomi erlaubt z.B., dass ihre Yeelight Lampen über ein \ac{IoT}-System per App, aber auch direkt über das \ac{LAN} steuerbar sind.\myfootcite[Vgl.][]{yeelight_docs}
...
Schließlich bleibt zu empfehlen alle von außerhalb eines Gebäudes erreichbaren Geräte in einem separaten Netzwerk zu organisieren, damit nicht auf das lokale Heimnetzwerk zugegriffen werden kann.
...
So können Befehle an ein Alexa Gerät weitergeben werden, welche über \ac{AWS} - Amazons Cloud - mit beliebigen Aktionen verknüpft werden können.
...
Für Besitzer von zertifizierten Geräten gibt es meist nur noch einen kurzen standardisierten Einrichtungsprozess zu überwinden.\myfootcite[Vgl.][]{workswithalexa}
...
Zusammenfassend bietet Home Assistant Privatsphäre, Unabhängigkeit gegenüber dem Internets, sowie eine hohe Bandbreite an verfügbaren Geräten und nutzbaren Schnittstellen.
...
Bei Betrachtung der Schätzungen in \autoref{tab:smarthome_market_predictions} ist die Idee des Verkaufs von \ac{IoT}-Geräten für Smart Homes so naheliegend, wie Schaufeln während eines Goldrauschs zu verkaufen.
...
Beim Verkauf von \ac{IoT}-Geräten für Smart Homes können verschiedene Ansätze gewählt werden.
...
Die ZigBee-Zertifizierung kostet z.B. 1000 USD pro Produkt.\myfootcite[Vgl.][]{zigbee_certification}
Alternativ kann das Produkt unter einer eigenen Marke vertrieben werden um genannte Kosten zu minimieren, was jedoch erhöhte Marketingkosten bedingt.
...
Hier wird auch klar, dass die Nutzung von \ac{HAC} einen Gewinn an Sicherheit bedeutet, da nicht wie herkömmlich über einen \ac{VPN} Vollzugriff auf das eigene Heimnetzwerk vergeben werden muss.\myfootcite[Vgl.][]{nabucasa}
...
Spätestens mit der Markteinführung von Amazon Alexa in 2014 wurde der Smart Home Trend global ausgelöst.
...
Viele \ac{IoT}-Produkthersteller implementieren \ac{IoT}-Funktionalitäten in ihre Produkte ohne sich über die Sicherheit ihrer Produkte Gedanken zu machen und darüber hinaus liefern die wenigsten Hersteller Sicherheitsaktualisierungen aus, falls eine Sicherheitslücke in ihrem Produkt bekannt wird.
...
Auch die Versicherungsbranche könnte mit in den Smart Home Markt involviert werden, indem z.B. Versicherungsprämien für eine Hausratversicherung reduziert werden, wenn mit dem Versicherer vernetzte Sicherheitssysteme eingesetzt werden.\myfootcite[Vgl.][]{smart_home_future}

Letztendlich bleibt es jedoch unklar wie sich der explizite Alltag mit Smart Homes in Zukunft gestalten wird, doch ähnelt die Entwicklung des Smart Home Trends der Entwicklung des Internets selbst.