...
% auch mesh, auch low energy, auch sicher
% zwave: geringere frequenz -> signal geht weiter, aber geringere bandbreit; nicht überall die selbe frequenz
% zigbee: 2,4ghz noise? höhere bandbreite aber kürzeres signal
% zwave ist kein offener standard -> eigener radio chip

Z-Wave lässt sich nahezu identisch, wie ZigBee beschreiben.
So wird auch eine Mesh Netzwerk-Topologie genutzt.
...
\autoref{fig:star_vs_mesh_network} vergleicht die Mesh-Topologie mit der klassischen Stern-Topologie, wie sie z.B. bei einem \ac{WLAN} zu finden ist.
...
\begin{table}[ht]
	...
	\begin{tabular}{| p{0.4\textwidth} | p{0.3\textwidth} | p{0.2\textwidth} |}
		...
		Verschlüsselung & \ac{AES}-128 & \ac{AES}-128 \\ \hline
		Netzwerk-Topologie & Mesh & Mesh \\ \hline
		Max. Anzahl an Geräten im Mesh & 232 & 65000 \\ \hline
		Genutzte Frequenzen & u.a. 868,4 MHz (EU); 908,4 MHz (US) & 2,4 Ghz \\ \hline
	\end{tabular}
	\caption*{\footnotesize{Quellen: \mycite[Vgl.][]{zwave_product_count,zigbee_faq,zwave_member_count,safewise_zwave_vs_zigbee,zwave_faq,smartcave_zwave_vs_zigbee,zwave_frequencies,zigbee_member_count}}}
	\label{tab:zwave_vs_zigbee}
\end{table}

\myfootcite[Vgl.][]{zwave_faq}
\myfootcite[Vgl.][]{zwave_alliance_overview}
\myfootcite[Vgl.][]{zwave_alliance_vision}
\myfootcite[Vgl.][]{zwave_plus_certification}
