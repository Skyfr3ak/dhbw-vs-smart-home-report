...
Aus der Leitebene leitet sich der Faktor Kontrolle ab, aus der Automatisierungsebene Automatisierungen und schließlich grundlegende Funktionalität aus der Feldbusebene.

Das primäre Risiko welches sich aus den vorhandenen Daten ableitet kann als nicht autorisierten Zugriff durch Dritte beschrieben werden.
Bspw. sollten die üblichen Aufenthaltszeiträume der Bewohner eines Smart Home Systems nicht für potentielle Einbrecher abrufbar sein.

% control

% criminals can open the door

% automations

% cat dies because she wasn't given any food

% basic functionality

% can't unlock the door on power loss

\subsubsection{Schwachstellen}

% where is data lost? -> cloud (third party), hack, wrong config (if used with iot applications)
% smart home device with security flaw can be used to break into the local network
% device with vulnerability can be used to hack into the system from the internet

\subsubsection{Risikoprävention}

% devices on a seperate network
% fallback devices
% no third-party deps -> hass
